...
def run_single_video_yolo8(model, source, tracker, output_folder, save_vid=False, save_vid2=False):
...
def run_yolo8(model, source, tracker, output_folder, save_vid=False, save_vid2=False):
    """

    Args:
        save_vid2: Создаем наше видео с центром человека
        save_vid: save для model.track. Yolo создает свое видео
    ...
    if source_path.is_dir():
        ...
        for entry in source_path.iterdir():
            ...
            if entry.is_file() and entry.suffix == ".mp4":
                run_single_video_yolo8(model, str(entry), tracker, output_folder, save_vid, save_vid2)
    else:
        run_single_video_yolo8(model, source, tracker, output_folder, save_vid, save_vid2)
